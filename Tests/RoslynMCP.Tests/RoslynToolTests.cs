...
public class RoslynToolTests
{
    ...
    public async Task LoadSolution_WithValidSolutionPath_ReturnsSuccessMessage()
    {
        ...
        var mockWorkspace = MSBuildWorkspace.Create();
        
        // Setup mock solution with projects
        ...
        var solution = mockWorkspace.CurrentSolution.AddProject(projectInfo);
        
        _mockWorkspaceService
            .Setup(ws => ws.GetWorkspaceAsync(solutionPath))
            .ReturnsAsync(mockWorkspace);
    ...
    public async Task LoadSolution_WithInvalidSolutionPath_ReturnsErrorMessage()
    {
        ...
        _mockWorkspaceService
            .Setup(ws => ws.GetWorkspaceAsync(invalidPath))
            .ThrowsAsync(new FileNotFoundException("Solution file not found"));
    ...
    [Test]
    public async Task GetSymbolInfo_WithValidLocation_CallsWorkspaceService()
    {
        // Arrange
        var solutionPath = Path.Combine(TestContext.CurrentContext.TestDirectory, "..", "..", "..", "..", "..", "TestSln", "TestSln.sln");
        solutionPath = Path.GetFullPath(solutionPath);
